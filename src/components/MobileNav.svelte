...
<div class="sm:hidden">
	...
	>
		{#if !isOpen}
			<svg
				...
				viewBox="0 0 24 24"
				class="fixed right-5 text-offWhite"
				transition:scale
			>
	...
	{#if isOpen}
		<nav
			class="fixed bottom-0 top-[80px] flex flex-col px-4 bg-darkBlueOpac text-offWhite h-[calc(100vh-80px)] w-full overflow-hidden z-40"
			in:fly={{ delay: 0, duration: 300, x: -800 }}
			out:fly={{ delay: 50, duration: 100, x: -800, opacity: 0 }}
		>
			<!-- h-[calc(100%-80px)] -->
			<ul
				class="flex flex-col items-center justify-center gap-2 mtpy-12 text-offWhite text-2xl overflow-hidden"
